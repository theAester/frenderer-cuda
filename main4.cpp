...
int main(int argc, char* argv[]){
	camera cam(vec3<double>(-15, 0, 0), vec3<double>(0,0,0), 72.6, 1); 
	scene sc(XRES,YRES,cam,vec3<double>(0.1,0.1,0.1));

	donut* don = new donut(vec3<double>(0,-5,0), vec3<double>(1.1,0,0), 1.8,2.8,true,4.4,4.4,2.4);
	donut* don2 = new donut(vec3<double>(0,5,0), vec3<double>(1.1,1.3,-0.4), 1.8,2.8,true,4.4,4.4,2.4);
	...
	while(true){
		...
		don2->change_angle(vec3<double>(1.1,1.3+c,1.1*c-0.4));
	}
	return 0;
}
